...
//删除节点，DOM会自动递归删除子节点，TreeNode递归手动删除子节点
	deleteNode: function () {
		...
		this.parent.selfElement.removeChild(this.selfElement);//移除对于的Dom节点
...
//位root绑定事件代理，处理所有节点的点击事件
addEvent(root, selfElement, "click", function(e){
	var target = e.target || e.srcElement;
	var domNode = target;
	while (document.className.indexOf("nodebody") == -1) domNode = domNode.parent;// 找到类名含有nodebody前缀的DOM结点
	...
	if(target.className.indexOf("node-title") != -1 || target.className.indexOf("arrow") != -1){
		selectedNOde.toggleFold();//触发toggle事件
		...
		for(var x=0;x<resultList.length;x++){
			...
			while (pathNode.parent != null) {
				if (pathNode.selfElement.className == "nodebody-hidden"){
					pathNode.parent.toggleFold();//若是收拢状态就展开
				}
